...
uint8_t NRF_Write_Reg(uint8_t reg, uint8_t value)
{
	uint8_t status;
	SPI_CSN_L();					  /* 选通器件 */
	status = SPI2_RWByte(reg);  /* 写寄存器地址 */
	SPI2_RWByte(value);		  /* 写数据 */
	SPI_CSN_H();					  /* 禁止该器件 */
  return 	status;
}
/*
...
uint8_t NRF_Read_Reg(uint8_t reg)
{
	uint8_t reg_val;
	SPI_CSN_L();					  /* 选通器件 */
	SPI2_RWByte(reg);			  /* 写寄存器地址 */
	reg_val = SPI2_RWByte(0);	  /* 读取该寄存器返回数据 */
	SPI_CSN_H();					  /* 禁止该器件 */
	return 	reg_val;
}
/*
