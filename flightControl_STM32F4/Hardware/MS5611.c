...
//设置超声波高度计优先。当超声波高度计有效时，不使用气压计的高度值。
...
void MS561101BA_getPressure(void) 
{
	...
	if (TEMP < 2000) // second order temperature compensation,温度补偿
	{  
		T2 = (((int64_t)dT)*dT) >> 31;
    ...
    MS5611_Pressure = MS561101BA_getAvg(Press_buffer,MOVAVG_SIZE); //0.01mbar，求平均值
	
	avg_Pressure = avg_Pressure + (MS5611_Pressure - avg_Pressure)*0.1f;//互补滤波
